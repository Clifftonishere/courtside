    ...
    req.on('end', () => {
      try {
        ...
        if (action === 'create-order') {
          ...
          console.log(`[${new Date().toISOString()}] LIMIT ORDER: ${side} ${size}x @ ${price} | token: ${tokenId.slice(0, 20)}...`);
          result = runPolymarket(cmd);

        } else if (action === 'market-order') {
          const { tokenId, side, amount } = data;
          if (!tokenId || !amount) {
            res.writeHead(400);
            res.end(JSON.stringify({ error: 'Missing tokenId or amount' }));
            return;
          }
          const cmd = `polymarket clob market-order --token "${tokenId}" --side ${side || 'buy'} --amount ${amount}`;
          console.log(`[${new Date().toISOString()}] MARKET ORDER: ${side} $${amount} | token: ${tokenId.slice(0, 20)}...`);
          result = runPolymarket(cmd);
